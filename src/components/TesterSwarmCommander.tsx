...
export const TesterSwarmCommander: React.FC<TesterSwarmCommanderProps> = ({
  ...
  const handleLaunchSwarm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage({
        type: 'success',
        text: `Swarm deployed! Successfully initialized ${data.count} bots (${baseName}1 to ${baseName}${data.count}) connecting to ${host}:${port}.`,
      });
      onRefresh();
    } catch (err: any) {
      setStatusMessage({
              ...
              <div className="lg:col-span-2 grid grid-cols-3 gap-2">
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-indigo-500/80 focus:ring-1 focus:ring-indigo-500/50 font-mono"
                    required
                  />
                </div>
                <div>
